...
|---|---|---|---|
| [`notebooks/00_verification.ipynb`](notebooks/00_verification.ipynb) | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/sear-labs/water-energy-coopt-scs-2021/blob/v1.0.2/notebooks/00_verification.ipynb) | **nothing** — no solver, no licence | is the published result correct? |
| [`notebooks/01_example.ipynb`](notebooks/01_example.ipynb) | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/sear-labs/water-energy-coopt-scs-2021/blob/v1.0.2/notebooks/01_example.ipynb) | a free solver | does the implementation run, and what does it do? |
...
results/tables/          COMMITTED cleaned output of the BASE model - the notebooks read this
results/tables/published/    COMMITTED aggregates of the FULL model, behind figures/
results/figures/         COMMITTED reproducible figures, from the base-model tables
figures/                 the PUBLISHED figures - a reader cannot regenerate these, see below
archive/                 FROZEN originals: the GAMS base model, the R that drew figures/,
                         and the extraction that produced results/tables/published/
model-gams/              FROZEN: the full co-optimisation model, GAMS source
MANIFEST.sha256          the fingerprint of everything frozen
```

**Three generated things are committed on purpose**, which is rule 5's documented exception:
`results/tables/` because without it nothing in the repository works from a clean clone — the
verification notebook would have no numbers to check; `results/tables/published/` because it is
the only way the paper's own aggregates are checkable without the restricted inputs; and
`results/figures/` plus `figures/` so a reader without a licence still sees what the prose refers
to.
...
**The published figures cannot be regenerated by a reader.** `figures/*.pdf` were drawn from the
raw output of the full co-optimisation model, run on Pecan Street–derived inputs. Those inputs
are not distributed — the data shipped here is **synthetic**, precisely to protect that dataset —
and the R workspace image the figures are drawn from is derived from them and is not distributed
either.

They can be regenerated *by the author*, from a private drive, and on 2026-09-11 they were: the
archived R was re-run against that image and reproduced six of the published figures bar for bar,
differing only in legend text metrics from a newer ggplot2. The committed PDFs were deliberately
left as they are — they are the artifacts of record. See [`archive/README.md`](archive/README.md).
...
**The per-scenario aggregates behind the figures ARE committed**, as of 2026-09-11. The archived
R computed costs, capacity, production and demand fractions and persisted them only into an R
workspace image; those frames are now exported to
[`results/tables/published/`](results/tables/published/) — 31 tables, 126 KB, every one indexed by
technology, scenario, month or fuel, with no household-level data, checked rather than assumed.
So the paper's aggregates *are* independently checkable from this repository, without R, GAMS or
the restricted inputs. What remains unreproducible is the step that produced them, not the
numbers.
